...
public:
	...
	// 09.05: Так как N является определённой при компиляции константой, то почему бы и не в компайл тайме проверить?
	//		  По крайне мере так пользователь заранее будет знать, что что-то не так (чем раньше тем лучше)
	Finite<N>& operator/=(const Finite<N>& num) {
		if (!is_prime(N)) {
...
public:
	...
	// Стоило бы это проверить, потому что пользователь не знает о таких правилах
		if (M != N) {
			...
			// Аналогично, можно было бы и в компайл тайме глянуть
		}
...
template<size_t M, size_t N, class Field = Rational>
Matrix<M, N, Field> operator+(Matrix<M, N, Field> a, const Matrix<M, N, Field>& b) { // a is copy
	a += b;
	return a;
}
...
template<size_t M, size_t N, class Field = Rational>
Matrix<M, N, Field> operator*(const Field& a, Matrix<M, N, Field> b) { // b is copy
	b *= a;
	return b;
}
...
Field row_to_col(const std::vector<Field>& row, const std::vector<Field>& col) { // check for equality
	size_t N = row.size();
	Field result = 0;
...
Matrix<M, K, Field> operator* (const Matrix<M, N, Field>& op_1, const Matrix<N, K, Field>& op_2) {
	...
	for (size_t i = 0; i < M; ++i) {
		for (size_t j = 0; j < K; ++j) {
			...
			//	std::cout << op_1.getRow(i)[k] << " ";
			//std::cout << " x ";
			//for (size_t f = 0; f < N; ++f)
			//	std::cout << op_2.getColumn(j)[f] << " ";
...
Field deter(Matrix < N, N, Field> matrix) {
	...
	for (size_t cur_row = 0; cur_row < N - 1; ++cur_row) {
		...
		if (matrix[new_index][cur_row] == 0) { // == 0 => 0 in every row => det = 0
			return 0;
		}
		...
		for (size_t dec = cur_row + 1; dec < N; ++dec) {
			...
			//std::cout << matrix[cur_row][cur_row];
			//std::cout << ",";
			//std::cout << coef;
